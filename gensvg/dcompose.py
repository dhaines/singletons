...
def latin(coord, note_names=None, accidentals=None):
    ...
    if accidentals == None:
        accidentals = ["𝄫𝄫", "♭𝄫", "𝄫", "♭", "", "♯", "𝄪", "♯𝄪", "𝄪𝄪"]
    acc = int(round(coord[1] / 7.0))
    return note_names[coord[1] - acc * 7 + 3] + accidentals[acc + 4]
...
class Button(object):
    ...
    def __repr__(self):
        return 'Button(coord="{0.coord}", name="{0.name}", octave={0.octave}, pitch={0.pitch}, pos={0.pos})'.format(
            self
        )
...
class DCompose(object):
    ...
    def pos(self, coord):
        return (
            coord[0] * self.xgen[0] + coord[1] * self.xgen[1],
    ...
    def get_notes(self):
        ...
        notes1 = [
            (-int(math.floor(f * fact + self.octave_start)), f)
            for f in range(self.min_note, self.max_note + 1)
        ]
        p = [self.pitch(n) for n in notes1]
        notes = []
        for o in range(
            int(math.floor((self.min_pitch - min(p)) / 1200)),
            int(math.ceil((self.max_pitch - max(p)) / 1200)) + 1,
        ):
    ...
    def get_buttons(self, latin=latin):
        ...
        buttons = [
            Button(n[:2], latin(n), n[2], self.pitch(n), self.pos(n)) for n in notes
        ]
        xs = [b.pos[0] for b in buttons]
        x_mean = (min(xs) + max(xs)) / 2.0
        x_shift = self.board_width / 2.0 - round(x_mean) + self.offset_x
        ys = [b.pos[1] for b in buttons]
        y_mean = (min(ys) + max(ys)) / 2.0
        y_shift = self.board_height / 2.0 - round(y_mean) + self.offset_y
        for b in buttons:
